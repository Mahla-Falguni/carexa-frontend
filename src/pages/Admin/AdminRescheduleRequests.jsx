...
import {
    ...
    FaCalendarAlt, FaClock, FaHospital, FaCommentAlt
} from "react-icons/fa";
...
const AdminRescheduleRequests = () => {
    ...
    return (
        <div className="min-h-screen bg-gradient-to-br from-slate-50 via-violet-50/20 to-indigo-50/10 p-6">
            <style>{`
                ...
                .card-row:hover { background: #f5f3ff; border-left-color: #7c3aed; transform: translateX(2px); }
            ...
            <div className="rs-page max-w-7xl mx-auto">
                ...
                <div className="mb-8 flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
                    <div>
                        ...
                        <p className="text-slate-500 text-sm">
                            View all patient reschedule requests — actions are handled by doctors
                        </p>
                    </div>
                ...
                <div className="grid grid-cols-2 sm:grid-cols-4 gap-4 mb-6">
                    {[
                        { label:"Total",    count:counts.ALL,      bg:"bg-violet-50",  icon:<FaExchangeAlt className="text-violet-600" size={16}/> },
                        { label:"Pending",  count:counts.PENDING,  bg:"bg-amber-50",   icon:<FaClock className="text-amber-600" size={16}/> },
                        { label:"Approved", count:counts.APPROVED, bg:"bg-emerald-50", icon:<FaCheckCircle className="text-emerald-600" size={16}/> },
                        { label:"Rejected", count:counts.REJECTED, bg:"bg-red-50",     icon:<FaTimesCircle className="text-red-500" size={16}/> },
                            ...
                            <table className="w-full table-fixed">
                                <thead>
                                    <tr className="bg-slate-50 border-b border-slate-100">
                                        <th className="text-left px-5 py-3 text-xs font-semibold text-slate-500 uppercase tracking-wider w-10">#</th>
                                        <th className="text-left px-5 py-3 text-xs font-semibold text-slate-500 uppercase tracking-wider w-44">Doctor</th>
                                        <th className="text-left px-5 py-3 text-xs font-semibold text-slate-500 uppercase tracking-wider w-36">Hospital</th>
                                        <th className="text-left px-5 py-3 text-xs font-semibold text-slate-500 uppercase tracking-wider w-28">Date</th>
                                        <th className="text-left px-5 py-3 text-xs font-semibold text-slate-500 uppercase tracking-wider w-24">Time</th>
                                        ...
                                        <th className="text-left px-5 py-3 text-xs font-semibold text-slate-500 uppercase tracking-wider w-16">View</th>
                                    </tr>
                                </thead>
                                        ...
                                        <tr key={req._id} className="card-row">
                                            ...
                                            <td className="px-5 py-4">
                                                <div className="flex items-center gap-2">
                                                    ...
                                                    <div className="min-w-0">
                                                        <p className="text-sm font-semibold text-slate-700 truncate">{req.appointment_id?.doctor_id?.name || "—"}</p>
                                                        <p className="text-xs text-slate-400 truncate">{req.appointment_id?.doctor_id?.specialization || ""}</p>
                                                    </div>
                                                </div>
                                            </td>
                                            <td className="px-5 py-4">
                                                <div className="flex items-center gap-1.5 text-sm text-slate-600">
                                                    ...
                                                    <span className="truncate">{req.appointment_id?.hospital_id?.hospital_name || "—"}</span>
                                                </div>
                                            </td>
                                            <td className="px-5 py-4">
                                                <div className="flex items-center gap-1.5 text-sm text-slate-600">
                                            ...
                                            <td className="px-5 py-4">
                                                <p className="text-sm text-slate-600 truncate" title={req.reason}>
                                                    {req.reason || <span className="italic text-slate-300">No reason</span>}
                                                </p>
                                            </td>
                                            <td className="px-5 py-4">
                                                <span className={`inline-flex items-center gap-1.5 px-2.5 py-1 text-xs font-semibold rounded-full border ${statusBadge(req.status)}`}>
                        ...
                        <div className="px-6 py-4 border-t border-slate-100 flex items-center justify-between">
                            ...
                            <div className="flex items-center gap-1.5">
                                <button onClick={() => setCurrentPage(p => Math.max(1, p - 1))} disabled={currentPage === 1}
                                    className="w-9 h-9 flex items-center justify-center border border-slate-200 bg-slate-50 hover:bg-slate-100 text-slate-500 text-sm rounded-lg disabled:opacity-30 transition">‹</button>
                                ...
                                <button onClick={() => setCurrentPage(p => Math.min(totalPages, p + 1))} disabled={currentPage === totalPages}
                                    className="w-9 h-9 flex items-center justify-center bg-violet-600 hover:bg-violet-700 text-white text-sm rounded-lg disabled:opacity-30 transition">›</button>
                            </div>
                        </div>
                    )}
                </div>
                    ...
                    <div className="bg-white rounded-2xl w-full max-w-md shadow-2xl overflow-hidden"
                        ...
                        <div className="bg-gradient-to-r from-violet-800 to-indigo-700 px-6 py-5 flex justify-between items-center">
                            <div>
                                ...
                                <p className="text-violet-300 text-xs mt-0.5">View only — actions managed by the doctor</p>
                            </div>
                        ...
                        <div className="p-6 space-y-3">
                            ...
                            <div className="bg-indigo-50 rounded-xl p-4 border border-indigo-100">
                                ...
                                <p className="text-sm text-slate-700"><span className="font-semibold">Name:</span> {selectedRequest.appointment_id?.doctor_id?.name || "—"}</p>
                                <p className="text-sm text-slate-700 mt-1"><span className="font-semibold">Specialization:</span> {selectedRequest.appointment_id?.doctor_id?.specialization || "—"}</p>
                            </div>
                            ...
                            <div className="bg-blue-50 rounded-xl p-4 border border-blue-100">
                                ...
                                <p className="text-sm text-slate-700">{selectedRequest.appointment_id?.hospital_id?.hospital_name || "—"}</p>
                            </div>
                            ...
                            <div className="bg-amber-50 rounded-xl p-4 border border-amber-100">
                                ...
                                <p className="text-sm text-slate-700">
                                    ...
                                    {selectedRequest.appointment_id?.appointment_date
                                        ? new Date(selectedRequest.appointment_id.appointment_date).toLocaleDateString("en-GB")
                                        : "—"}
                                </p>
                                <p className="text-sm text-slate-700 mt-1">
                                    <span className="font-semibold">Time:</span>{" "}
                                    {selectedRequest.appointment_id?.start_time || "—"}
                                </p>
                            </div>
                            ...
                            <div className="px-3 py-2 bg-slate-100 rounded-xl">
                                <span className="text-xs text-slate-500">
                                    ℹ️ Approve/Reject actions are handled by the assigned doctor from their dashboard.
                                </span>
                            </div>
                        </div>

                        {/* ✅ Close only — NO approve/reject buttons */}
